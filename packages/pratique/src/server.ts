...
export class Server {
    ...
    get<TPath extends string>(path: TPath, ...handlers: [...Middleware<TPath>[], Handler<TPath>]) {
        this.addHandler('GET', path, ...handlers);
    }

    post<TPath extends string>(path: TPath, ...handlers: [...Middleware<TPath>[], Handler<TPath>]) {
        this.addHandler('POST', path, ...handlers);
    }

    put<TPath extends string>(path: TPath, ...handlers: [...Middleware<TPath>[], Handler<TPath>]) {
        this.addHandler('PUT', path, ...handlers);
    }

    patch<TPath extends string>(path: TPath, ...handlers: [...Middleware<TPath>[], Handler<TPath>]) {
        this.addHandler('PATCH', path, ...handlers);
    }

    delete<TPath extends string>(path: TPath, ...handlers: [...Middleware<TPath>[], Handler<TPath>]) {
    ...
    private applyMiddlewares(req: Request) {
        for (const middleware of this.middlewares) {
            middleware({ request: req, queryParams: new URLSearchParams(req.url.split('?')[1]) });
        }
    }
